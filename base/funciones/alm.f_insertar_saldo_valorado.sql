CREATE OR REPLACE FUNCTION alm.f_insertar_saldo_valorado (
  p_fecha date
)
RETURNS boolean AS
$body$
...
BEGIN
	 ...
	 v_nombre_funcion = 'alm.f_insertar_saldo_valorado';

     /*create temp table ttsaldo_valorado_item(
     	id_item integer,
    	id_almacen integer,
    	fecha_hasta date,
    	valorado numeric
     ...
     for v_resultado in select ti.id_item, ti.codigo, ti.nombre, ta.id_almacen, ta.codigo, ta.nombre
     	...
     	v_valorado = alm.f_get_saldo_valorado_item(v_resultado.id_item, v_resultado.id_almacen, p_fecha);

        insert into alm.tsaldo_valorado_item (
        	id_item,
            id_almacen,
            fecha_hasta,
            valorado
